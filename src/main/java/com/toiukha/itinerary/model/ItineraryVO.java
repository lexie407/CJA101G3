...
public class ItineraryVO {
    ...
    @Column(name = "CRTID", nullable = false, updatable = false) // 對應資料庫欄位 "crtId"，不可為空 (FK)
    private Integer crtId; // 建立會員

    @Column(name = "ITNDESC", length = 500) // 對應資料庫欄位 "itnDesc"，長度為500
    private String itnDesc; // 行程描述

    @Column(name = "ISPUBLIC", nullable = false, insertable = false) // 對應資料庫欄位 "isPublic"，不可為空
    private Byte isPublic; // 是否公開 (0=私人, 1=公開)

    @Column(name = "ITNSTATUS", nullable = false, insertable = false) // 對應資料庫欄位 "itnStatus"，不可為空
